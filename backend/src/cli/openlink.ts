...
async function cmdStart(args: string[]) {
  ...
  if (daemonMode) {
    await processManager.spawnDaemon(
      'node',
      [backendDist],
      PID_FILE,
      LOG_FILE,
      { cwd: PROJECT_ROOT, env: { ...process.env, NODE_ENV: 'production' } }
    );
...
async function cmdStatus() {
  ...
  const portOpen = await processManager.isPortOpen(GATEWAY_PORT);
...
async function cmdDev() {
  ...
  const prefix = (name: string, color: string) => (data: Buffer) => {
  ...
  backend.on('exit', (code) => {
    if (code !== 0 && code !== null) console.log(`Backend exited with code ${code}`);
  });
  frontend.on('exit', (code) => {
    if (code !== 0 && code !== null) console.log(`Frontend exited with code ${code}`);
  });
}
